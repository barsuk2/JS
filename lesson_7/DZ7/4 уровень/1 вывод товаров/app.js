...
btn.forEach(function (arg) {
    arg.addEventListener('click', clickHandler)
})
...
function clickHandler(event) {
    div_prod.innerHTML = '';
    let type_prod = event.target.getAttribute('data-type')
    showCategory(type_prod)
...
function getProductMarkup(product) {
    for (let i of product) {
        div_prod.insertAdjacentHTML('beforeend',
            `<div class="product">
             <div>${i.name}</div>-->
             <img src="${i.imageUrl}" alt="">
                <div>${i.price}</div>
                 <a href="https://example.com/producs/${i.id}">Подробнее</a></div>`)
    }
    console.log(product)
}